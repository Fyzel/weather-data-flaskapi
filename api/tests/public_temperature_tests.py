...
class TestCasePublicTemperature(unittest.TestCase):
    @property
    def token(self):
        return self._token
    ...
    def setUp(self):
        ...
        self.resource = 'PublicTemperatureData'
    ...
    def test_step_00_get_all_records_without_auth(self):
        ...
        TestCasePublicTemperature.last_id = None

        if len(json_data) > 0:
            TestCasePublicTemperature.last_id = json_data[0]['id']
    ...
    def test_step_01_get_record_without_auth(self):
        ...
        if TestCasePublicTemperature.last_id is not None:
            app_url = '{base_url}/{context}/{resource}/{last_id}'.format(
                base_url=self.base_url,
                context=self.context,
                resource=self.resource,
                last_id=TestCasePublicTemperature.last_id
            )
