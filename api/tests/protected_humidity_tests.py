...
class TestCaseProtectedHumidity(unittest.TestCase):
    @property
    def token(self):
        return self._token
    ...
    def test_step_01_login(self):
        ...
        TestCaseProtectedHumidity.token = json_data['access_token']
    ...
    def test_step_03_create_record_with_auth(self):
        ...
        headers = {
            'content-type': 'application/json',
            'authorization': 'JWT {token}'.format(token=TestCaseProtectedHumidity.token),
            'cache-control': 'no-cache'
        }
        ...
        TestCaseProtectedHumidity.last_id = json_data['id']
        TestCaseProtectedHumidity.previous_record = json_data
    ...
    def test_step_04_get_record_without_auth(self):
        ...
        app_url = '{base_url}/{context}/{resource}/{last_id}'.format(
            ...
            context=self.context,
            resource=self.resource,
            last_id=TestCaseProtectedHumidity.last_id
        )
    ...
    def test_step_05_get_record_with_auth(self):
        ...
        self.assertEqual(
            json_data['value'],
            TestCaseProtectedHumidity.previous_record['value']), 'Returned value is the same'

        self.assertEqual(
            json_data['value_units'],
            TestCaseProtectedHumidity.previous_record['value_units']), 'Returned value_units is the same'

        self.assertEqual(
            json_data['value_error_range'],
            TestCaseProtectedHumidity.previous_record['value_error_range']), 'Returned value_error_range is the same'

        self.assertEqual(
            json_data['timestamp'],
            TestCaseProtectedHumidity.previous_record['timestamp']), 'Returned timestamp is the same'

        self.assertEqual(
            json_data['elevation'],
            TestCaseProtectedHumidity.previous_record['elevation']), 'Returned elevation is the same'

        self.assertEqual(
            json_data['elevation_units'],
            TestCaseProtectedHumidity.previous_record['elevation_units']), 'Returned elevation_units is the same'

        self.assertEqual(
            json_data['latitude'],
            TestCaseProtectedHumidity.previous_record['latitude']), 'Returned latitude is the same'

        self.assertEqual(
            json_data['latitude_public'],
            TestCaseProtectedHumidity.previous_record['latitude_public']), 'Returned latitude_public is the same'

        self.assertEqual(
            json_data['longitude'],
            TestCaseProtectedHumidity.previous_record['longitude']), 'Returned longitude is the same'

        self.assertEqual(
            json_data['longitude_public'],
            TestCaseProtectedHumidity.previous_record['longitude_public']), 'Returned longitude_public is the same'

        self.assertEqual(
            json_data['city'],
            TestCaseProtectedHumidity.previous_record['city']), 'Returned city is the same'

        self.assertEqual(
            json_data['province'],
            TestCaseProtectedHumidity.previous_record['province']), 'Returned province is the same'

        self.assertEqual(
            json_data['country'],
            TestCaseProtectedHumidity.previous_record['country']), 'Returned country is the same'
