...
class TestCasePressure(unittest.TestCase):
    @property
    def token(self):
        return self._token
    ...
    def test_step_01_login(self):
        ...
        TestCasePressure.token = json_data['access_token']
    ...
    def test_step_03_create_record_with_auth(self):
        ...
        headers = {
            'content-type': 'application/json',
            'authorization': 'JWT {token}'.format(token=TestCasePressure.token),
            'cache-control': 'no-cache'
        }
        ...
        TestCasePressure.last_id = json_data['id']
        TestCasePressure.previous_record = record_data
    ...
    def test_step_04_get_record_without_auth(self):
        ...
        app_url = '{base_url}/{context}/{resource}/{last_id}'.format(
            ...
            context=self.context,
            resource=self.resource,
            last_id=TestCasePressure.last_id
        )
        ...
        self.assertEqual(
            int(json_data['id']),
            int(TestCasePressure.last_id)), "Returned id is the same"
        self.assertEqual(
            json_data['value'],
            TestCasePressure.previous_record['value']), 'Returned value is the same'
        self.assertEqual(
            json_data['value_units'],
            TestCasePressure.previous_record['value_units']), 'Returned value_units is the same'
        self.assertEqual(
            json_data['value_error_range'],
            TestCasePressure.previous_record['value_error_range']), 'Returned value_error_range is the same'
        self.assertEqual(
            json_data['timestamp'],
            str(TestCasePressure.previous_record['timestamp'].strftime(
                '%Y-%m-%dT%H:%M:%S'))), 'Returned timestamp is the same'
        self.assertEqual(
            json_data['elevation'],
            TestCasePressure.previous_record['elevation']), 'Returned elevation is the same'
        self.assertEqual(
            json_data['elevation_units'],
            TestCasePressure.previous_record['elevation_units']), 'Returned elevation_units is the same'
        self.assertEqual(
            json_data['latitude'],
            TestCasePressure.previous_record['latitude']), 'Returned latitude is the same'
        self.assertEqual(
            json_data['longitude'],
            TestCasePressure.previous_record['longitude']), 'Returned longitude is the same'
