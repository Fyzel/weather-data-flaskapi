...
class TestCaseHumidity(unittest.TestCase):
    @property
    def token(self):
        return self._token
    ...
    def test_step_01_login(self):
        ...
        TestCaseHumidity.token = json_data['access_token']
    ...
    def test_step_03_create_record_with_auth(self):
        ...
        headers = {
            'content-type': 'application/json',
            'authorization': 'JWT {token}'.format(token=TestCaseHumidity.token),
            'cache-control': 'no-cache'
        }
        ...
        TestCaseHumidity.last_id = json_data['id']
        TestCaseHumidity.previous_record = record_data
    ...
    def test_step_04_get_record_without_auth(self):
        ...
        app_url = '{base_url}/{context}/{resource}/{last_id}'.format(
            ...
            context=self.context,
            resource=self.resource,
            last_id=TestCaseHumidity.last_id
        )
        ...
        self.assertEqual(
            int(json_data['id']),
            int(TestCaseHumidity.last_id)), "Returned id is the same"
        self.assertEqual(
            json_data['value'],
            TestCaseHumidity.previous_record['value']), 'Returned value is the same'
        self.assertEqual(
            json_data['value_units'],
            TestCaseHumidity.previous_record['value_units']), 'Returned value_units is the same'
        self.assertEqual(
            json_data['value_error_range'],
            TestCaseHumidity.previous_record['value_error_range']), 'Returned value_error_range is the same'
        self.assertEqual(
            json_data['timestamp'],
            str(TestCaseHumidity.previous_record['timestamp'].strftime(
                '%Y-%m-%dT%H:%M:%S'))), 'Returned timestamp is the same'
        self.assertEqual(
            json_data['elevation'],
            TestCaseHumidity.previous_record['elevation']), 'Returned elevation is the same'
        self.assertEqual(
            json_data['elevation_units'],
            TestCaseHumidity.previous_record['elevation_units']), 'Returned elevation_units is the same'
        self.assertEqual(
            json_data['latitude'],
            TestCaseHumidity.previous_record['latitude']), 'Returned latitude is the same'
        self.assertEqual(
            json_data['longitude'],
            TestCaseHumidity.previous_record['longitude']), 'Returned longitude is the same'
